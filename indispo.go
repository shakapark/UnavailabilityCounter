...
func newMonth(instance string, monthName string, y int){
...
func register(result int, groupName string){
	var path string
	...
	if result == 0 {
		...
		if indi.Progress == false {
			...
			if indi.StartTimeStamp.Month() != indi.TimeStampBack.Month() {
				newMonth(groupName, indi.StartTimeStamp.Month().String(), indi.StartTimeStamp.Year())
			}
		...
		if indi.Progress == true {
			...
			path = "/data/"+groupName+"/"+indi.StartTimeStamp.Month().String()+strconv.Itoa(indi.StartTimeStamp.Year())
